...
public class Scheduling_a_correlated_message
{
    ...
    public async Task Should_automatically_configure_correlation()
    {
        ...
        await harness.Bus.Publish(new StartInstance(id));
        ...
        MassTransitStateMachine<Instance>
    {
        public InstanceStateMachine()
        {
            ...
            Initially(
                When(Start)
                    .Schedule(Notify, context => new NotifyInstance(context.Message.CorrelationId))
    ...
    public record StartInstance(Guid CorrelationId);


    public record NotifyInstance(Guid CorrelationId) : CorrelatedBy<Guid>;
}
